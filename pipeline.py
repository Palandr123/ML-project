...
from utils import search_sequence_numpy


class SelfGuidanceSDXLPipeline(StableDiffusionXLPipeline):
    def get_sg_aux(self, apply_tree_map: bool = True, transpose: bool = True) -> dict[str, dict[str, torch.Tensor]]:
        """
            ...
            try:
                ...
                if transpose:
                    for key, value in module_auxiliary.items():
                        if apply_tree_map: 
                            value = torch.utils._pytree.tree_map(lambda vv: vv.chunk(2)[1] if vv is not None else None, value)
                        auxiliary_data[key][name] = value
                else:
                    auxiliary_data[name] = module_auxiliary
                    if apply_tree_map:
                        auxiliary_data[name] = {k: torch.utils._pytree.tree_map(lambda vv: vv.chunk(2)[1] if vv is not None 
                                                                                else None, v) for k, v in auxiliary_data[name].items()}
